...
|---|---|---|
| Durable session restart | Encrypted session survives restart and continues the same conversational focus | Pending |
| History pagination | Agent retrieves an older required turn by opaque cursor without duplicating recent context | Pending |
...
| Privacy boundary | Provider storage disabled; persisted conversation and paused state encrypted | Pending |
