alter table public.work_sessions
  drop constraint work_sessions_recovery_source_check,
  add constraint work_sessions_recovery_source_check check (
    not is_recovery or source_session_id is not null
  );
...
create function public.work_session_continuation_snapshot(
...
create function public.read_work_session_continuation(
  p_intent_id uuid,
  p_version integer
)
returns jsonb
...
begin
  ...
  if (
    ...
    or p_next_stage not in (
      'offer', 'choosing', 'confirming', 'declined'
    )
    or p_action is null
...
create function public.confirm_work_session_continuation(
  p_update_id bigint,
  p_owner_chat_id bigint,
  p_owner_id text,
  p_intent_id uuid,
...
begin
  ...
  if (
    current_intent.id is null
    or current_intent.version <> p_version
    or current_intent.stage <> 'confirming'
...
revoke all on function public.work_session_continuation_snapshot(
  public.work_session_continuation_intents
) from public, anon, authenticated;
revoke all on function public.read_work_session_continuation(
  uuid, integer
) from public, anon, authenticated;
...
revoke all on function public.confirm_work_session_continuation(
  bigint, bigint, text, uuid, integer, timestamptz, timestamptz
) from public, anon, authenticated;

grant execute on function public.read_work_session_continuation(
  uuid, integer
) to service_role;
...
grant execute on function public.confirm_work_session_continuation(
  bigint, bigint, text, uuid, integer, timestamptz, timestamptz
) to service_role;
